...
%. jl_exec(S) is det.
%  Execute Julia expression in S, which may be string, atom, codes or chars S.
...
%. jl_eval(S, X) is det.
%  Evaluate Julia expression in S, which may be string, atom, codes or chars S.
...
%  If any Julia exceptions are raised, the Julia function =|showerror|= is used
%  to print the details, and a Prolog exception is raised.

%. !(E:expr) is det.
%  Execute Julia expression E. No output is printed.
